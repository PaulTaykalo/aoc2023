...
// find coords of S
var startCoord: Coord = Coord(x: 0, y: 0)
var maxWidth = 0
var maxHeight = 0
...
func findAvailableMoves(for coord: Coord) -> [Coord] {
    let moves = _indAvailableMoves(for: coord)
        // Filter cooords are withing map
    return moves
    .filter { $0.x >= 0 && $0.y >= 0 && $0.x <= maxWidth && $0.y <= maxHeight}
    .filter { map[$0.y][$0.x] != "." }
}

func _indAvailableMoves(for coord: Coord) -> [Coord] {
    switch map[coord.y][coord.x] {
        case "S": 
           return [coord.top(), coord.bottom(), coord.left(), coord.right()]        
        case "L":
            return [coord.top(), coord.right()]
        case "F":
            return [coord.right(), coord.bottom()]    
        case "J":
            return [coord.top(), coord.left()]
        case "7":
            return [coord.bottom(), coord.left()]
        case "-":
...
let resul = findAvailableMoves(for: startCoord)
var path: [Coord] = [startCoord]
...
while !wave.isEmpty {
    ...
    let current = wave
    current.forEach { visited.insert($0) }

    var next = Set(current.flatMap { findAvailableMoves(for: $0) }
    ...
    // for v in next {
    //   print("[\(steps)]Coord InMap\n\(coordOnMap(v)) )")
    // }

    if next.isEmpty {
        print("Last step: \n\(nearby(coord: wave.first!))")
        break
    }
...
// once we found all the coords we need to find sum of all tiles that are not part of the path, and not touching board corners
var unchecked: Set<Coord> = Set()

for y in 0...maxHeight {
    for x in 0...maxWidth {
        let coord = Coord(x: x, y: y)
        if !visited.contains(coord) {
            unchecked.insert(coord)
        }
    }
}
...
var originalUnchecked = unchecked

print("Total unchecked: \(unchecked.count)")
// var fields: [Set<Coord>] = []

// while unchecked.count > 0 {
//     let current = unchecked.first!
//     unchecked.remove(current)

//     var field = Set([current])
//     var wave = [current]

//     while !wave.isEmpty {
//         let current = wave

//         let next = Set(current.flatMap { [$0.top(), $0.left(), $0.right(), $0.bottom(), $0.top().left(), $0.top().right(), $0.bottom().left(), $0.bottom().right()] })
//             .filter { !field.contains($0) }
//             .filter { !visited.contains($0) }
//             .filter { unchecked.contains($0) }
//             .filter { $0.x >= 0 && $0.y >= 0 && $0.x <= maxWidth && $0.y <= maxHeight}
...
//     if field.allSatisfy({ $0.x > 0 && $0.y > 0 && $0.x < maxWidth && $0.y < maxHeight }) {
//         print("Field: \(field)")
//         totalSum += field.count
...
var mmap = map
// Update S to be 7
mmap[startCoord.y][startCoord.x] = "7"

let spath = Set(path)


func numberOfIntesectionsToTheRightFor(coord: Coord) -> Int {
    var intersections = 0
    var current = coord
    var lastChar: Character? = nil
    while current.x < maxWidth {
        current.move(.right)
        guard spath.contains(current) else { continue }
        let char = mmap[current.y][current.x]
        ...
        // We dont' actually care, since we'll jump into one of the others
        if char == "-" {
            continue
        }

        if lastChar == nil && ( char == "F" || char == "J" || char == "7" || char == "L" ) {
            lastChar = char
            intersections += 1
            continue
        }

        if lastChar == "F" && char == "7" {
            lastChar = nil
            intersections += 1
            continue
        }

        // Moved up (so F was the intersection)
        if lastChar == "F" && char == "J" {
            lastChar = nil
            continue
        }

        if lastChar == "L" && char == "J" {
            lastChar = nil
            intersections += 1
            continue
        }
...
// Calculate intersections for all original unchecked

var totalSumByIntersections = 0
for coord in originalUnchecked.sorted(by: { $0.x < $1.x }) {
    let intersections = numberOfIntesectionsToTheRightFor(coord: coord)
    // print("Coord: \(coord), intersections: \(intersections) \(intersections % 2 == 1 ? "✅" : "❌")")
    if intersections % 2 == 1 {
        totalSumByIntersections += 1
    }
    print("Counted: \(coord)")
}
...
print("Updated sum: \(totalSumByIntersections)")


func coordOnMap(_ coord: Coord) -> String {
    var map = map
    map[coord.y][coord.x] = "X"
    return map.map { String($0) }.joined(separator: "\n")
}

func nearby(coord: Coord) -> String {
    return String([
        coord.top().left(),
        coord.top(), 
        coord.top().right(),
