...
var totalAvailableGames = 0
var sumOfAvailableGamesIDs = 0
var powerSum = 0
for game in games {
    let gameNumber = Int(game.split(separator: ":")[0].split(separator: " ")[1])!
    print("Game \(game)")
    let moves = game.split(separator: ":")[1]
    let movesArray = moves.split(separator: ";")
    var maxRed = 0
    var maxGreen = 0
    var maxBlue = 0

    for move in movesArray {
        let colorsArray = move.split(separator: ",")
        for color in colorsArray {
            let colorArray = color.split(separator: " ")
            let colorName = String(colorArray[1])
            let colorValue = Int(colorArray[0])!
    ...
    //     totalAvailableGames += 1
    //     sumOfAvailableGamesIDs += gameNumber
    // } else {
    //     let redMessage = maxRed <= red ? nil : "Red: not enough \(maxRed) > \(red)"
    //     let greenMessage = maxGreen <= green ? nil : "Green: not enough \(maxGreen) > \(green)"
    //     let blueMessage = maxBlue <= blue ? nil : "Blue: not enough \(maxBlue) > \(blue)"
    //     // print("Game \(gameNumber) is not available. Reason: not enough items of each color. \(redMessage ?? ""), \(greenMessage ??), Blue: \(blueMessage)")
    // }
}

print("Total available games: \(totalAvailableGames)")
print("Sum of available games IDs: \(sumOfAvailableGamesIDs)")
