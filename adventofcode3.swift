...
var sum_of_inundjusted =  0

struct Coord: Hashable, Equatable {
    var x: Int
    var y: Int
}
var adjancency_to_star = [Coord: [Int]]()
...
for yy in 0..<map.count {
    
    var is_number = false
    var is_adjanced = false
    var numberChars = [Character]()
    var adjanced_to_stars = [Coord]()
    
    for xx in 0..<map[yy].count {
        let char = map[yy][xx]
        if char.isNumber {
            is_number = true
            ...
            if is_number {
                let number = Int(String(numberChars))!
                print("number: \(number) : \(is_adjanced))")
                numberChars.removeAll()
                if is_adjanced {
                    sum_of_inundjusted += number
                }
                
                if adjanced_to_stars.count > 0 {
                    // leave only unique values adjanced_to_stars
                    var unique = [Coord]()
                    for coord in adjanced_to_stars {
                    ...
                    for coord in unique {
                        adjancency_to_star[coord, default:[]].append(number)
                    }
                }
                adjanced_to_stars.removeAll()
        ...
        if is_number {
            ...
            // chec all around if there's not a number
            ...
            for i in -1...1 {
                for j in -1...1 {
                    ...
                    // print("x: \(x), y: \(y) char: \(map[y][x])")
                    if !map[y][x].isNumber && map[y][x] != "."   {
                        print("\(map[yy][xx]) Is adjanced! to \(map[y][x])")
                        is_adjanced = true
                    }
                    
                    if (map[y][x] == "*") {
                        print("\(map[yy][xx]) Is adjanced! to \(map[y][x])")
                        adjanced_to_stars.append(Coord(x: x, y: y))
                    }
                }
            }
        }
...
print(adjancency_to_star)

var sum_of_adjanced_stars = 0
for (coord, numbers) in adjancency_to_star {
    // if numbers.count == 2 {
        print("coord: \(coord) numbers: \(numbers)")
        // sum_of_adjanced_stars += numbers[0] * numbers[1]
    // }
}

print(sum_of_adjanced_stars)
